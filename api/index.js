...
Api.prototype.getServiceEnvironments = function(service, next) {
  ...
  etcd.get(key, function(err, keyData) {
    if (err) return next(err);

    var environments = _.sortBy(_.map(_.pluck(keyData.node.nodes, 'key'), function(item) { return item.replace(key, '')}), function(env) {
      switch (env) {
      case 'local':
        return 1;
      case 'development':
    ...
    // Add heartbeats
    etcd.get(heartbeat, function(getErr, hbData) {
      var heartbeats = {};

      _.each(hbData.node.nodes, function(item) {
